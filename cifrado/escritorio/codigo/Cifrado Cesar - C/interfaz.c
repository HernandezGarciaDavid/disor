...
GtkWidget *mensaje,*desplazamiento,*rCesar,*rAlrevez,*ralrv3,*grupos;
void imprime(){
  //char* algo;
  ...
  resultado3[i]='\0';
  alrevez(resultado2);
  char *grup=gtk_entry_get_text(GTK_ENTRY(grupos));
  if(strlen(grup)>0){
    int valorDesplazamiento;
    sscanf(grup,"%i",&valorDesplazamiento);
    alrv(resultado3,valorDesplazamiento);
...
void accionCesar(){
  ...
  if(strlen(algo)>0){
    int valorDesplazamiento;
    sscanf(gtk_entry_get_text(GTK_ENTRY(desplazamiento)),"%i",&valorDesplazamiento);
    cesar(resultado1,valorDesplazamiento);
    gtk_entry_set_text(GTK_ENTRY(rCesar),resultado1);
  }
}
void accionAlr(){
  ...
  if(strlen(algo)>0){
    int valorDesplazamiento;
    sscanf(gtk_entry_get_text(GTK_ENTRY(grupos)),"%i",&valorDesplazamiento);
    alrv(resultado1,valorDesplazamiento);
    gtk_entry_set_text(GTK_ENTRY(ralrv3),resultado1);
  }
}
static void
activate (GtkApplication *app,
          gpointer        user_data)
{
  GtkWidget *window,*etiquetas[6],*cesar,*grid[4],*frame[3];
  ...
  g_signal_connect (desplazamiento, "key_release_event",
                  G_CALLBACK (accionCesar), NULL);
  ...
  etiquetas[4]=gtk_label_new("Grupo:");
  gtk_grid_attach(GTK_GRID(grid[3]),etiquetas[4],0,0,1,1);
  grupos=gtk_entry_new();
  gtk_grid_attach(GTK_GRID(grid[3]),grupos,1,0,1,1);
  etiquetas[5]=gtk_label_new("Resultado");
  gtk_grid_attach(GTK_GRID(grid[3]),etiquetas[5],0,1,1,1);
  ralrv3=gtk_entry_new();
  gtk_grid_attach(GTK_GRID(grid[3]),ralrv3,1,1,1,1);
